...
bosque_plant_week_combinations <- fruitset_census %>%
  dplyr::select(Bosque,Planta,X,Y,Week_ISO,Periodo) %>%
  unique()
...
for(combination_i in 1:nrow(bosque_plant_week_combinations)){
  ...
  total_pollinator_i_data_clogit <- total_pollinator_i_data_clogit_raw %>%
    filter(Bosque == bosque_plant_week_combinations$Bosque[combination_i],
           ...
           Week_ISO == bosque_plant_week_combinations$Week_ISO[combination_i],
  ...
  if(nrow( total_pollinator_i_data_clogit)>1){
    ...
    potential_steps_i <- potential_steps_to_subplot(
      ...
      Week_ISO_i = bosque_plant_week_combinations$Week_ISO[combination_i],
...
sum(is.na(bosque_plant_week_combinations$prob_consp_step))/nrow(bosque_plant_week_combinations) # 0.2849315
