...
interactions_data20 <- read_csv("results/donana/foraging_donana_2020.csv") %>%
  mutate(Codigo_vuelo = as.character(Codigo_vuelo))
interactions_data21 <- read_csv("results/donana/foraging_donana_2021.csv")
...
for (Year.i in unique(pollination$Year)){
  ...
  pollination_bosque_i <- pollination %>% filter(Year==Year.i) %>% ungroup() %>%
    dplyr::select(Bosque) %>% unique() %>% pull()
  
  for (Bosque_i in pollination_bosque_i){
    ...
    folder_base <- paste0(dir_ini,"/results/donana/Multilayer_Species/")
    ...
    if(Bosque_i=="Pinar Villamanrique Este (Chaparral)"){
      list_files_field_level <- files_base[grepl("Chaparral", files_base) &
    ...
    ##############
    # PAgeRAnk Multilayer
    saveRDS(graph_Plot_i, file = paste0("results/donana/NN_networks/Bosque_",Bosque_i,"_Year_",Year.i,"_NN_intra_inter.rds"))
