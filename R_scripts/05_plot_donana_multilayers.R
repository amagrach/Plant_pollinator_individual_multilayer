...
set.seed(123)
vector_colores=sample(col_vector, 60)
pie(rep(1,60), col=vector_colores)

min_between_poll_space_finder <- function(Year.i,Bosque_i){
  
  # Load multilayer----------
  file_i <- paste0("results/donana/NN_networks/Bosque_",Bosque_i,"_Year_",Year.i,"_NN_intra_inter.rds")
  mult_i <- readRDS(file_i)
  
  nodes <- V(mult_i)$name
  nodes_prop <- tibble(name = nodes) %>% separate(name, c("node", "layer")," ")
  nodes_prop$type <- NA
  nodes_prop$type[!grepl("-",nodes_prop$node, fixed = TRUE)] <- "visitor"
  nodes_prop$type[grepl("-",nodes_prop$node, fixed = TRUE)] <- "plant"
...
candidates_for_between <- function(plant_per_species,between){
  ...
  for(j in test_denominator){
    second_limit_candidates <- c(second_limit_candidates,opt_radius(max_edge_aux/j))
  }
...
circumscribe_polygon <- function(list_nodes){
  ...
  test_denominator <- cumsum(c(0.01,rep(0.001,10000)))
...
plot_multilayer <- function(Year.i,Bosque_i,between_poll_space, r_ext){
  # Load multilayer----------
  file_i <- paste0("results/donana/NN_networks/Bosque_",Bosque_i,"_Year_",Year.i,"_NN_intra_inter.rds")
  mult_i <- readRDS(file_i)
  ...
  aux_layer <- sub("_","",nodes_prop$layer)
  nodes_prop$type[!grepl("-", nodes_prop$node, fixed = TRUE)] <- "visitor"
  nodes_prop$type[grepl("-", nodes_prop$node, fixed = TRUE)] <- "plant"
  ...
  # Fix plant layer
  #nodes_prop$layer[nodes_prop$type != "visitor"] <- nodes_prop$node[nodes_prop$type != "visitor"]
  ...
  fitness_data2 <- read_csv(paste0("results/donana/foraging_donana_",Year.i,".csv"))
  plant_list <- fitness_data2$Planta %>% unique()
  plant_list <- plant_list[!is.na(plant_list)] 
  
  colors_plants <- tibble(layer = sort(plant_list),#plant_list,
                          color = vector_colores[1:length(plant_list)])
  ...
  if(sum(list_lengths)-2*max(list_lengths) < 0){
    
    max_edge <- max(list_lengths)
    
    extra_nodes <- ceiling((2*max_edge-sum(list_lengths))/nrow(plant_per_species))
  ...
  # Fix layer from in plants
  # aux_edge_layer <- sub("_","",edge_prop$layer_from)
  # edge_prop$layer_from[numbers_only(aux_edge_layer)] <- edge_prop$node_from[numbers_only(aux_edge_layer)]
  
  # aux_edge_layer <- sub("_","",edge_prop$layer_to)
  # edge_prop$layer_to[numbers_only(aux_edge_layer)] <- edge_prop$node_to[numbers_only(aux_edge_layer)]
  ...
  sp_plant_names <- tibble(layer = plant_list,
                        sp_name = sub("_", "\n", plant_list))
...
png("figures/donana_Pinar_Aznalcazar_2020.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,4)+.1,xpd=TRUE)
plot_multilayer(2020,"Pinar Aznalcazar",between_poll_space=10, r_ext= 60)
dev.off()
...
plot_multilayer(2020,"Pinar Hinojos",between_poll_space=15, r_ext= 100)
dev.off()

png("figures/donana_Pinar_Hinojos_2021.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2021,"Pinar Hinojos",between_poll_space=15, r_ext= 60)
dev.off()

png("figures/donana_Pinar_Puebla_2020.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2020,"Pinar Puebla",between_poll_space=15, r_ext= 60)
dev.off

png("figures/donana_Pinar_Puebla_2021.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2021,"Pinar Puebla",between_poll_space=15, r_ext= 60)
dev.off()

png("figures/donana_Pinar_Villamanrique_Este_2021.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2021,"Pinar Villamanrique Este (Chaparral)",between_poll_space=15, r_ext= 150)
dev.off()

png("figures/donana_Villamanrique_Chaparral_2020.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2020,"Villamanrique Chaparral",between_poll_space=15, r_ext= 200)
dev.off()

png("figures/donana_Villamanrique_Sur_2020.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2020,"Villamanrique Sur",between_poll_space=10, r_ext= 90)
dev.off()

png("figures/donana_Pinar_Villamanrique_Sur_2021.png", width=1000*3, height = 500*3, res=300)
par(mar=c(0,0,0,14)+.1,xpd=TRUE)
plot_multilayer(2021,"Pinar Villamanrique Sur",between_poll_space=15, r_ext= 60)
dev.off()