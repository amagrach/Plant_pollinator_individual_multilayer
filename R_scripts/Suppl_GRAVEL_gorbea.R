...
flights_raw$Periodo[flights_raw$Year==2020 & flights_raw$Day_ISO > 149] <- 2
flights_raw$Periodo[flights_raw$Year==2021 & flights_raw$Day_ISO > 127] <- 2
flights_raw$Periodo[flights_raw$Year==2021 & flights_raw$Day_ISO > 152] <- 3
...
unique(total_visits_pollinator_subplot$Polinizador) %>% sort()

pollinator_species <- c("Bombus_pascuorum","Apis_mellifera",
...
# "results/gorbea/rd_visits_subplot_pollinator_plant.csv") # Commented for security reasons

rd_visits_subplot_pollinator_plant <- read_csv("results/gorbea/rd_visits_subplot_pollinator_plant.csv")
...
not_enoug_observations <- observations_planta$Planta[observations_planta$n<4]
...
for(pol_i in 1:length(planta_fits)){
  ...
  plant_m <- glmmTMB(percentil ~ 1+(1|Periodo/Year),
...
for(pol_i in 1:length(planta_fits)){
  ...
  plant_m <- glmmTMB(number_flowers ~ 1+(1|Periodo/Year),
                          family = nbinom1(),
                          data = data_i)
...
colors_viridis <- viridis(8, option = "D")


colors_brewer_extra <- brewer.pal(8, "Dark2")

colors_combined <- c(colors_brewer, colors_viridis, colors_brewer_extra)
colors_combined <- colors_combined[1:25]

# Crear el gráfico
png("figures/Gorbeia_visits_abundance.png",
...
ggplot(df_plant_visits_fl_ab, aes(x = mean_flores, y = mean_percentil, color = position)) +
  ...
  geom_hline(yintercept = 97.5, linetype="dashed")+
...
unique(df_plant_visits_fl_ab$position)
