...
generate_UPDATED_rd_steps <- function(steps_pollinator_i_year_plot_periodo_change_row,
                              number_random_steps,
                              steps_length_data,
  ...
  if(length(different_step_lengths)<2){
    ...
    shape_aux <- (fit[["estimate"]][["shape"]]+tentative_shape_coef)
    
    rate_aux <- (fit[["estimate"]][["rate"]]-tentative_rate_coef)
    
    if(shape_aux<0){shape_aux <- 1e-3}
    if(rate_aux<0){rate_aux <- 1e-3}
