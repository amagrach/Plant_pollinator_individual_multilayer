...
# INPUT (1) -> visit_list: it contains (Plot,Polinizador,Subplot_Plant_Label(e.g. A1 LEMA),Visits_tot)
# OUTPUT (1) -> list with motifs: it contains
# (bosque_id,number_nodes,number_plants,plant_1,subplot_plant_1,
...
motifs_extraction <- function(visit_list) {
  ...
  for (bosque_i in 1:length(list_incid_matrix)){
    ...
    if (length(motifs)>0){
      ...
      motif_3$bosque_id <- as.numeric(names_plot[bosque_i])
      ...
      for (i in 1:length(motifs)){
        ...
        for (j in 1:length(lista_names_i)) {
          ...
          if (lista_names_i[j] %in%  visit_list$Subplot_Plant_Label){
            ...
            list_plants <- c(list_plants,c(location_plant[[1]][1]))
            list_plants_loc <- c(list_plants_loc,c(location_plant[[1]][2]))
...
Planta_pheno_overlap <- function(Planta1,Planta2,interactions) {
  
  Plantasx <- sort(unique(interactions$Planta))
  if(sum(c(Planta1,Planta2) %in% Plantasx)<2){
    print(paste("Error: At least one plant does not belong to bosque ",
                Bosque_i," experimental phenology",sep=""))
  }else{
    pollination_Planta1 <- interactions %>% filter(Planta==Planta1)
    #Weeks in which plant 1 recieves visits
    Planta_1_weeks <- min(pollination_Planta1$Week):max(pollination_Planta1$Week)
    
    pollination_Planta2 <- interactions %>% filter(Planta==Planta2)
    #Weeks in which plant 2 recieves visits
    Planta_2_weeks <- min(pollination_Planta2$Week):max(pollination_Planta2$Week)
    
    #number of weeks with phenological overlap between plant1 and plant2
    overlap <- sum(Planta_1_weeks %in% Planta_2_weeks)
    
    #total_number of weeks in the experimental phenology of plot i
    total_weeks_1 <-  length(Planta_1_weeks)
